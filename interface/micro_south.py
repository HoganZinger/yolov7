...
def detect_batch(indir, outdir, button, bar, window):
    ...
    weights = './micro/pretrain_0.2_nohist.pt'
    classifier = './micro/cls.pkl'
    detect('tusizi', bar, window, device, indir, outdir, foldername, weights, classifier)
...
def select_img(var, label, window, button, button2):
    ...
    weights = './micro/pretrain_0.2_nohist.pt'
    classifier = './micro/cls.pkl'
    res = detect_one('tusizi', img, device, weights, classifier)
...
def show_interface_1(window):
    ...
    bar = Progressbar(interface_1_window)
    # 按钮，点击则开始一次批量检测
    start_detect = Button(interface_1_window, text='开始检测', width=15, height=2, command=lambda: thread_it(detect_batch, result_dict['file_dict'], result_dict['save_path'], start_detect, bar, interface_1_window)) # 点击按钮执行一# 个名为“hit_me”的函数
    # 按钮，选择待测图片所在文件夹
    folder = Button(interface_1_window, text="待测图片文件夹", width=15, height=2, command=lambda: thread_it(select_folder, var1, l1, folder, interface_1_window))
    # lable，说明输入框作用
    var2 = StringVar()
    l5 = Label(interface_1_window,
    ...
    title = Label(interface_1_window,
        text="南方菟丝子栅状细胞显微检测",    # 标签的文字
        bg="#f0f0f0",     # 标签背景颜色
        font=('宋体', 16),     # 字体和字体大小
    ...
    # 保存图片按钮
    save = Button(interface_1_window, text="保存", width=15, height=2, font=('宋体', 10), command=lambda: thread_it(save_img, interface_1_window, save))
    save['state'] = DISABLED

    # 选择图片并检测的按钮
    img = Button(interface_1_window, text="待测图片", width=15, height=2, font=('宋体', 10), command=lambda: thread_it(select_img, var3, l3, interface_1_window, img, save))
    ############################################################### 单张检测组件声明结束
    ...
    change = Button(interface_1_window, text="换为单张检测", width=10, height=1, font=('宋体', 10), command=lambda: thread_it(change_type, change, batch_objs, single_objs, interface_1_window))
    change.place(relx=0.015, rely=0.022)
