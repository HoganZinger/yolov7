...
def detect_batch(indir, outdir, button, bar, window, results_list, result_select):
    ...
    weights = './micro/micro_stage1.pt'
    classifier0 = './micro/surface_02_resnet50_1.pkl'
    classifier1 = './micro/side4_02_resnet50_1.pkl'
    classifier2 = './micro/neizhongpi_01_resnet50_1.pkl'
...
def show_interface_2():
    ...
    interface_2_window.geometry('1200x900')
    interface_2_window.resizable(False, False)  # 禁止最大化

    # 加载 PNG 图像
    bg_path = '..background/background.png'
    bg_image = Image.open(bg_path)
    bg_photo = ImageTk.PhotoImage(bg_image)
    # 创建一个 Canvas
    canvas = tk.Canvas(interface_2_window, width=400, height=300)
    ...
    prev_result = ttk.Button(interface_2_window, text='上一条', width=8, command=lambda:
                             thread_it(change_selection, result_select, 'prev'))
    next_result = ttk.Button(interface_2_window, text='下一条', width=8, command=lambda:
                             (change_selection, result_select, 'next'))
    report = ttk.Button(interface_2_window, text='生成检测报告', width=15, command=lambda:  thread_it(show_report_window,
                        interface_2_window, report, result_dict, pre_list, path_list))
    #################################################### 批量检测组件声明结束
    ...
    ############################################################## 批量检测组件布置
    title.place(relx=0.34, rely=0.07)
    folder.place(relx=0.22, rely=0.23)
    # save_folder.place(relx=0.22, rely=0.24)
    start_detect.place(relx=0.22, rely=0.36)
    results_list.place(relx=0.70, rely=0.22)
    result_select.place(relx=0.43, rely=0.54)
    category_select.place(relx=0.43, rely=0.64)
    confirm.place(relx=0.72, rely=0.64)
    prev_result.place(relx=0.22, rely=0.84)
    next_result.place(relx=0.78, rely=0.84)
    report.place(relx=0.45, rely=0.80)
    ############################################################### 批量检测组件布置结束
