...
def detect_batch(indir, outdir, button, bar, window):
    ...
    detect('surface', bar, window, device, indir, outdir, foldername, 'micro_stage1.pt', './pickles/surface_02_resnet50_1.pkl')
...
def select_img(var, label, window, button, button2):
    ...
    device = torch.device("cpu" if torch.cuda.is_available() else "cpu")  # 系统能提供的device
    res = detect_one('surface', img, device, 'micro_stage1.pt', './pickles/surface_02_resnet50_1.pkl')
...
def show_interface_3():
    interface_3_window = tk.Toplevel()
    interface_3_window.title("酸枣仁栅状细胞表面观显微结构检测")
    interface_3_window.geometry('1200x800')
    interface_3_window.resizable(False, False)  # 禁止最大化

    ########################################################### 批量检测组件声明
    # lable，说明输入框作用
    var1 = StringVar()
    l1 = Label(interface_3_window,
    ...
    bar = Progressbar(interface_3_window)
    # 按钮，点击则开始一次批量检测
    start_detect = Button(interface_3_window, text='开始检测', width=15, height=2,
                          command=lambda: thread_it(detect_batch, result_dict['file_dict'], result_dict['save_path'],
                                                    start_detect, bar, interface_3_window))  # 点击按钮执行一# 个名为“hit_me”的函数
    # 按钮，选择待测图片所在文件夹
    folder = Button(interface_3_window, text="待测图片文件夹", width=15, height=2,
                    command=lambda: thread_it(select_folder, var1, l1, folder, interface_3_window))
    # lable，说明输入框作用
    var2 = StringVar()
    l5 = Label(interface_3_window,
    ...
    # 选择保存路径的按钮
    save_folder = Button(interface_3_window, text="保存路径", width=15, height=2,
    ...
    title = Label(interface_3_window,
                  text="酸枣仁栅状细胞表面观显微结构检测",  # 标签的文字
                  bg="#f0f0f0",  # 标签背景颜色
                  font=('宋体', 16),  # 字体和字体大小
    ...
    # 保存图片按钮
    save = Button(interface_3_window, text="保存", width=15, height=2, font=('宋体', 10),
                  command=lambda: thread_it(save_img, interface_3_window, save))
    save['state'] = DISABLED

    # 选择图片并检测的按钮
    img = Button(interface_3_window, text="待测图片", width=15, height=2, font=('宋体', 10),
                 command=lambda: thread_it(select_img, var3, l3, interface_3_window, img, save))
    ############################################################### 单张检测组件声明结束
    ...
    change = Button(interface_3_window, text="换为单张检测", width=10, height=1, font=('宋体', 10),
                    command=lambda: thread_it(change_type, change, batch_objs, single_objs, interface_3_window))
    change.place(relx=0.015, rely=0.022)
