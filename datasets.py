...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            if img0.shape[-1] > 3:
                img0 = img0[:, :, 0:3].astype(np.uint8)
            assert img0 is not None, 'Image Not Found ' + path

            # ############################################################################################################ start equalization
            # # cores use HSI hist
            # # convert to HSI first
            # hsi_img = cv2.cvtColor(img0, cv2.COLOR_BGR2HSV)  # change PIL image to cv2 image
            #
            # # only need to equalize I channel
            # H, S, I = cv2.split(hsi_img)
            # I = cv2.equalizeHist(I)
            #
            # # conver back to RGB
            # hsi_img = cv2.merge((H, S, I))
            # img0= cv2.cvtColor(hsi_img, cv2.COLOR_HSV2BGR)
            # ############################################################################################################ end equalization
