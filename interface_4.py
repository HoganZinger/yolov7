...
def detect_batch(indir, outdir, button, bar, window):
    ...
    detect('side', bar, window, device, indir, outdir, foldername, 'micro_stage1.pt', './pickles/side4_02_resnet50_1.pkl')
...
def select_img(var, label, window, button, button2):
    ...
    device = torch.device("cpu" if torch.cuda.is_available() else "cpu")  # 系统能提供的device
    res = detect_one('side', img, device, 'micro_stage1.pt', './pickles/side4_02_resnet50_1.pkl')
